...
st.image('mouse_lemur_muscles_small.jpg', caption='Musculoskeletal model of the mouse lemur')
...
st.markdown('''
Muscles can insert on large areas and can have complex arrangements in 3D. To be modelled, they hence should be discretised into a **series of strands** that represent their **lines of action**. The way these springs are set on the bones should reflect the area of origin and insertion of the muscle. Anatomical observations based on dissections and scans are therefore important to make modelling choices, and understand the limitations of the model (for instance see [Goening et al. 2013]( https://royalsocietypublishing.org/doi/full/10.1098/rsif.2013.0216?rfr_dat=cr_pub++0pubmed&url_ver=Z39.88-2003&rfr_id=ori%3Arid%3Acrossref.org)]. In our model, we will represent the muscles as straight lines connecting the origin (on the cranium) and insertion (on the mandible) areas.
